...
jQuery(function($) {
			...
			if (!$w.parent(document.body).length) {
				...
				$w = $('<iframe style="position:absolute; top:-9999px; left:-9999px" />')
					.attr({
						id: z,
						name: z
					}).attr('src', t).appendTo(document.body);
			} else $w.attr('src', t);
	...
	pidAjaxFileUpload = function() {
		...
		$('<img src="./common/img/msg.loading.gif" style="display:none;">')
			.ajaxStart(function() {
				$(this).show();
			}).ajaxComplete(function() {
				$(this).hide();
			}).appendTo($('#fUloader').find('.fpv').empty());
		...
		function on_complete(ret) {
			...
			if (fs && fs.length) {
				for (i = 0, c = fs.length; i < c; i++) {
					...
					$('<option id="sif' + r + '" value="' + r + '" data-src="' + x + '"></option>')
	...
	$('#fUloader:eq(0)').each(function() {
		...
		$('.scFup', this).click(function() {
			$('.fd').find('input[type=file]:first').trigger('click');
			return false;
		});
		...
		$('#siFiles', this).change(function() {
			var t, r = $('#siFiles').val(),
				src = $("#sif" + r).attr('data-src'),
				u = src || '';
			if (!u.match(/\.(?:(jpe?g|png|gif))$/i)) {
				if (u) t = '<img src="./modules/editor/tpl/images/files.gif" />';
				else t = '<img src="./common/img/blank.gif" />';
			} else {
				t = '<img src="' + u + '" />';
			}
			$(t).appendTo($('#fUloader').find('.fpv').empty());
		});
	});
});
